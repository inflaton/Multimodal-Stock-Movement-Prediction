% Reviewer-response tags used throughout this source:
% [R1-LIT]        Adds the requested recent evidence-acquisition literature.
% [R1-SCOPE]      Bounds the five-equity, single-test-year empirical claim.
% [R1-SENTIMENT]  Qualifies the FinBERT-Tone daily mean-pooled representation.
% [R1-FOUNDATION] Qualifies the asymmetric, single-seed foundation-model study.
% [R1-FUSION]     Limits the learned-fusion claim to one convex weight.
% [R1-PRIOR]      Clarifies that the prior-result comparison is a reconstruction.
% [R1-CAMERA]     Records post-acceptance camera-ready production changes.
...
% [R1-CAMERA] Omit hyperlink annotations and unused drawing/color packages for the final IEEE PDF.
\usepackage{url}
\usepackage{booktabs}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{graphicx}
% [R1-CAMERA] The unused xcolor package is intentionally omitted from the camera-ready source.
\usepackage{multirow}
\usepackage{cite}
\usepackage{balance}
% [R1-CAMERA] Unused TikZ support and the removed hyperlink setup are intentionally absent here.

\begin{document}

\title{Does Sentiment Fusion Help?\\A Selection-Bias-Free Re-Evaluation of Multimodal Stock Movement Prediction}

% [R1-CAMERA] Include the accepted author identities and affiliations in the non-anonymous version.
\author{
\IEEEauthorblockN{
Donghao HUANG\textsuperscript{1},
...
% [R1-PRIOR] Replace causal-sounding attribution to prior papers with a bounded reconstruction claim.
\emph{First}, selecting the best model and horizon per stock on the test set
inflates the average Sharpe ratio from 1.40 to 2.29, a 39\% increase that puts
an otherwise identical pipeline in the range of some prior reports.
\emph{Second}, under bias-free
...
underperform technical-only; the gap is $0.58$ Sharpe on average for fixed 70:30
...
% [R1-LIT] Add the specifically requested recent work on evidence acquisition for multimodal financial reasoning.
\noindent\textbf{Evidence acquisition for multimodal financial reasoning.}
Recent work shifts from monolithic fusion toward actively acquiring the
evidence needed for each reasoning step. Deng et al.\ propose SEA, a
planner-guided multi-agent framework that decomposes financial questions and
retrieves task-conditioned evidence from images, text, and external knowledge
before answering \cite{deng2026sea}. Their setting is complementary to ours:
SEA studies evidence-grounded multimodal question answering, whereas we test
whether pre-aggregated daily sentiment improves out-of-sample movement
prediction. It nevertheless motivates event- or query-conditioned text
representations as a richer alternative to daily mean pooling.
...
% [R1-PRIOR] State the trial count for our actual reconstruction rather than implying a direct prior-code reproduction.
...
horizon---but the ``best'' is identified on the test year. Even our grid of
seven model families and three candidate horizons creates $21$ trials per
stock; the maximum over noisy trials is an upward-biased estimate of true
...
% [R1-PRIOR] Mark the estimate as a controlled mechanism reconstruction, not a reproduction of a named prior paper.
$2.29$ average Sharpe ratio---squarely in the range of some prior reports---while
the identical pipeline, selected honestly, yields $1.40$. Roughly $0.9$ units
of Sharpe ratio, about $39\%$ of the test-peek value, is attributable to the
selection rule alone. This is a controlled reconstruction of the mechanism,
not a direct reproduction of any specific prior paper.
...
% [R1-SCOPE] Make the five-equity and one-year test boundary explicit.
claims. (i)~\textbf{Universe:} five large-cap U.S.\ equities and a single test
year (2024); the negative result may not transfer to other regimes, small caps,
or other markets, though the selection-bias mechanism is general.
% [R1-SENTIMENT] State that the null may be specific to daily mean-pooled FinBERT-Tone features.
(ii)~\textbf{Sentiment representation:} one daily FinBERT-Tone scalar per
source; document embeddings, event tags, intraday text, or step-wise evidence
acquisition \cite{deng2026sea} could preserve signal that mean pooling discards.
(iii)~\textbf{Data quality:} SPY news coverage in the test year is only $41\%$,
so SPY's sentiment ablations are partly imputed; the conclusion is unchanged
when SPY is excluded.
% [R1-FOUNDATION] Identify both the single-seed evidence and FinCast input asymmetry.
(iv)~\textbf{Foundation-model comparison:} these models
were run single-seed under compute limits, and the released FinCast artifact
accepts only prices, so this comparison is directional rather than conclusive.
% [R1-FUSION] Restrict the negative learned-fusion result to the tested one-parameter family.
(v)~\textbf{Fusion class:} our learned method is a one-parameter convex weight;
its failure does not rule out richer cross-modal architectures.
% [R1-PRIOR] Explicitly separate selection-bias reconstruction from direct prior-code reproduction.
(vi)~\textbf{Prior-work comparison:} the $2.29$ test-peek result reconstructs a
selection mechanism in our pipeline rather than rerunning another paper's code;
it demonstrates the size of the bias but does not attribute any specific prior
result to that mechanism.
...
% [R1-CAMERA] Do not claim that an internal checklist is part of the published manuscript package.
through.
