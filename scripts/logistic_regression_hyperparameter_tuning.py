"""
Logistic Regression Hyperparameter Tuning for Stock Prediction

This script performs Bayesian hyperparameter optimization for Logistic Regression models.
It automatically finds optimal price movement thresholds for each prediction horizon.
...
Usage:
    python logistic_regression_hyperparameter_tuning.py --stock AAPL --data-dir /path/to/data --n-calls 30
    python logistic_regression_hyperparameter_tuning.py --all-stocks --strategy long_only_confidence
    python logistic_regression_hyperparameter_tuning.py --stock NVDA --horizons 5 7 10
"""
...
# ============================================================================
# Logistic Regression Tuning Function
# ============================================================================


def tune_logistic_regression_for_horizon(
    X_train, y_train, X_val, y_val, n_calls=30, verbose=True
):
    """
    Tune Logistic Regression hyperparameters using Bayesian optimization.
    ...
    def objective(C, penalty, l1_ratio):
...
def run_tuning_for_stock(
    ...
    """
    Run Logistic Regression hyperparameter tuning for all horizons of a given stock.
    """
    print(f"\n{'='*70}")
    print(f"LOGISTIC REGRESSION HYPERPARAMETER TUNING: {stock}")
    print(f"{'='*70}")
    ...
    for horizon, threshold in horizons_thresholds:
        ...
        best_params, val_auc, _ = tune_logistic_regression_for_horizon(
            X_train_tune,
            y_train_tune,
            X_val_tune,
            y_val_tune,
...
def main():
    parser = argparse.ArgumentParser(
        description="Logistic Regression Hyperparameter Tuning for Stock Prediction"
    )
    parser.add_argument(
    ...
    for strategy in strategies_to_run:
        ...
        if args.all_stocks:
            ...
            combined.to_csv(
                f"{args.output_dir}/LogisticRegression_tuned_all_stocks_results{strategy_suffix}.csv", index=False
            )
            print(
                f"\nCombined results saved to: {args.output_dir}/LogisticRegression_tuned_all_stocks_results{strategy_suffix}.csv"
            )
        else:
