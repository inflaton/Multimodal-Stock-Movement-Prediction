"""
LightGBM Hyperparameter Tuning for Stock Prediction

This script performs Bayesian hyperparameter optimization for LightGBM models.
...
Usage:
    python lightgbm_hyperparameter_tuning.py --stock AAPL --data-dir /path/to/data --n-calls 30
    python lightgbm_hyperparameter_tuning.py --all-stocks --strategy long_only_confidence
"""
...
def run_tuning_for_stock(
    ...
    """
    Run LightGBM hyperparameter tuning for all horizons of a given stock.
    """
    print(f"\n{'='*70}")
    print(f"LIGHTGBM HYPERPARAMETER TUNING: {stock}")
    print(f"{'='*70}")
    ...
    sanitized_cols = sanitize_feature_names(feature_cols)
    ...
    for horizon, threshold in horizons_thresholds:
        ...
        # Prepare features with sanitized names
        X_train = pd.DataFrame(train_copy[feature_cols].values, columns=sanitized_cols)
        y_train = train_copy["target"].values
        X_test = pd.DataFrame(test_copy[feature_cols].values, columns=sanitized_cols)
        y_test = test_copy["target"].values

        # Split training into train/val for tuning
        val_size = int(len(X_train) * 0.2)
        X_train_tune = X_train.iloc[:-val_size]
        y_train_tune = y_train[:-val_size]
        X_val_tune = X_train.iloc[-val_size:]
        y_val_tune = y_train[-val_size:]

        # Tune hyperparameters
        best_params, val_auc = tune_lightgbm_for_horizon(
            X_train_tune,
            y_train_tune,
            X_val_tune,
            y_val_tune,
    ...
    output_file = f"{output_dir}/LightGBM_tuned_{stock}_results{strategy_suffix}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="LightGBM Hyperparameter Tuning for Stock Prediction"
    )
    parser.add_argument(
    ...
    for strategy in strategies_to_run:
        ...
        if args.all_stocks:
            ...
            combined.to_csv(
                f"{args.output_dir}/LightGBM_tuned_all_stocks_results{strategy_suffix}.csv", index=False
            )
            print(
                f"\nCombined results saved to: {args.output_dir}/LightGBM_tuned_all_stocks_results{strategy_suffix}.csv"
            )
        else:
