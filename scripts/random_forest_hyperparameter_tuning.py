"""
Random Forest Hyperparameter Tuning for Stock Prediction

This script performs Bayesian hyperparameter optimization for Random Forest models.
...
Usage:
    python random_forest_hyperparameter_tuning.py --stock AAPL --data-dir /path/to/data --n-calls 30
    python random_forest_hyperparameter_tuning.py --all-stocks --strategy long_only_confidence
"""
...
def tune_random_forest_for_horizon(
...
def run_tuning_for_stock(
    ...
    """
    Run Random Forest hyperparameter tuning for all horizons of a given stock.
    """
    print(f"\n{'='*70}")
    print(f"RANDOM FOREST HYPERPARAMETER TUNING: {stock}")
    print(f"{'='*70}")
    ...
    for horizon, threshold in horizons_thresholds:
        ...
        best_params, val_auc = tune_random_forest_for_horizon(
            X_train_tune,
            y_train_tune,
            X_val_tune,
            y_val_tune,
...
def main():
    parser = argparse.ArgumentParser(
        description="Random Forest Hyperparameter Tuning for Stock Prediction"
    )
    parser.add_argument(
    ...
    for strategy in strategies_to_run:
        ...
        if args.all_stocks:
            ...
            combined.to_csv(
                f"{args.output_dir}/RandomForest_tuned_all_stocks_results{strategy_suffix}.csv", index=False
            )
            print(
                f"\nCombined results saved to: {args.output_dir}/RandomForest_tuned_all_stocks_results{strategy_suffix}.csv"
            )
        else:
