"""
Gradient Boosting Hyperparameter Tuning for Stock Prediction

This script performs Bayesian hyperparameter optimization for Gradient Boosting models.
...
Usage:
    python gradient_boosting_hyperparameter_tuning.py --stock AAPL --data-dir /path/to/data --n-calls 30
    python gradient_boosting_hyperparameter_tuning.py --all-stocks --strategy long_only_confidence
"""
...
# ============================================================================
# Gradient Boosting Tuning Function
# ============================================================================


def tune_gradient_boosting_for_horizon(
...
def run_tuning_for_stock(
    ...
    """
    Run Gradient Boosting hyperparameter tuning for all horizons of a given stock.
    """
    print(f"\n{'='*70}")
    print(f"GRADIENT BOOSTING HYPERPARAMETER TUNING: {stock}")
    print(f"{'='*70}")
    ...
    for horizon, threshold in horizons_thresholds:
        ...
        best_params, val_auc = tune_gradient_boosting_for_horizon(
            X_train_tune,
            y_train_tune,
            X_val_tune,
            y_val_tune,
    ...
    output_file = f"{output_dir}/GradientBoosting_tuned_{stock}_results{strategy_suffix}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Gradient Boosting Hyperparameter Tuning for Stock Prediction"
    )
    parser.add_argument(
    ...
    for strategy in strategies_to_run:
        ...
        if args.all_stocks:
            ...
            combined.to_csv(
                f"{args.output_dir}/GradientBoosting_tuned_all_stocks_results{strategy_suffix}.csv", index=False
            )
            print(
                f"\nCombined results saved to: {args.output_dir}/GradientBoosting_tuned_all_stocks_results{strategy_suffix}.csv"
            )
        else:
