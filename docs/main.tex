...
Oriani and Coelho~\cite{oriani2016evaluating} demonstrated that combining different indicators as inputs to the same neural network can further improve forecasting performance. Building on this insight, we identify a compact set of robust \emph{indicator pairs} via systematic backtesting over the training period (2020--2023). For each stock independently, we evaluate all pairwise combinations of base indicators across prediction horizons $h \in \{2,\dots,10\}$ days using a long-short trading strategy with 10~bps transaction costs. Pairs are ranked by cumulative return, and the top 10 combinations are selected as features for that stock. This asset-specific selection allows each model to exploit the technical patterns most predictive for that particular security, while the multi-horizon evaluation ensures robustness across different holding periods.
